...
class DashboardController extends Controller {
  ...
  public function postmiscpay() {
    ...
    $line->CreateDate = date('Y-m-d H:i:s');
  ...
  public function searchinvoice($id) {
    ...
    // $inv=DB::select('select il.InvoiceHeaderID,sh.CustomerID ,sum(il.Amount)Amount,isnull(sum(rl.Amount),0)Paid
    //   from Invoicelines il 
    //   join ServiceHeader sh on il.ServiceHeaderID=sh.ServiceHeaderID
    //   left join ReceiptLines rl on rl.InvoiceHeaderID=il.InvoiceHeaderID
    //   where il.InvoiceHeaderID='.$id.'
    //   group by il.InvoiceHeaderID,sh.CustomerID');

    //   dd($inv);
    
    try {
      // $inv = Invoice::find(intval($id));
      // dd($inv);

      $inv=DB::select('select il.InvoiceHeaderID,sh.CustomerID,c.CustomerName ,sum(il.Amount)Amount,isnull(sum(rl.Amount),0)Paid,
      sum(il.Amount)-isnull(sum(rl.Amount),0)Balance
      from Invoicelines il 
      join ServiceHeader sh on il.ServiceHeaderID=sh.ServiceHeaderID
    join customer c on sh.CustomerID=c.CustomerID
      left join ReceiptLines rl on rl.InvoiceHeaderID=il.InvoiceHeaderID
      where il.InvoiceHeaderID='.$id.'
      group by il.InvoiceHeaderID,sh.CustomerID,c.CustomerName');

      $balance=$inv[0]->Balance;
      $CustomerID=$inv[0]->CustomerID;
      
      //dd($balance);

      //$cust = Customer::find(intval($inv->CustomerID));
      $cust = Customer::find(intval($CustomerID));
      $invoice = [
        'invoice' => $id,
        'status' => 'done',
        'balance' => $balance,
