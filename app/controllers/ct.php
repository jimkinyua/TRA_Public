  ...
  public function applicationform($cat) {
    if($cat == 0) {
      ...
      ->select(['Inspections.InspectionID',
        'ServiceHeader.ServiceHeaderID', 
        'Inspections.InspectionStatusID', 
        'Inspections.UserID',
        ...
        'Users.UserFullNames',
        'Inspections.UserComment'])
      ->where('ServiceHeader.CustomerID', Session::get('customer')->CustomerID)
      ->join('Customer','Customer.CustomerID','=','ServiceHeader.CustomerID')
      ->join('Services','Services.ServiceID','=','ServiceHeader.ServiceID')
      ->join('ServiceStatus','ServiceStatus.ServiceStatusID','=','ServiceHeader.ServiceStatusID')
      ->join('Inspections','Inspections.ServiceHeaderID','=','ServiceHeader.ServiceHeaderID')
      ->join('Users','Users.AgentID','=','Inspections.UserID')
      ->pluck('InspectionStatusID');
    ...
    return View::make('applications/form', ['ServiceStatusID' => $ServiceStatusID, 'bill' => $bill, 'services' => $services, 'form' => $form,'categoryName'=>$categoryName,'docs'=>$docs, 'categoryID'=>$categoryID ]);
  }
