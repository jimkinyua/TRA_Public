  ...
  <head>
    ...
    <script type="text/javascript">
      ...
      var filterActivity = function() {
        ...
        if(x == 0) { return; }
        console.log('filtering...', el);

        var url = "/categoryservices/" + x;
        $.get(url, function(res, status){
          $("#category").empty(); // remove old options
          $.map(res.data, function(value,key) { $("#category").append($("<option></option>").attr("value", value).text(key)); });
        });
      }
    </script>
    ...
    <div id="landing" class="ui grid">
      ...
      <header id="header" class="row">
        <h3 id="heading" class="ui center aligned header">
          ...
          <div class="content">
            THE COUNTY GOVERNMENT OF UASIN GISHU SELF SERVICE PORTAL
