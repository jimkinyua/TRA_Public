...
@section('script')
    ...
    @if($invoice->paid() > 0)
      <?php
        $paid = $invoice->paid();
        $total += $invoice->receipted();
        $its = $invoice->items()->get();
        foreach ($its as $key => $item) {
          ...
          $gross += $item_amnt;
          array_push($items, [ ($invoice->id() . '#' . $item->id()), ($item->service->ServiceName . ' ' . $item->Description), number_format($item_amnt, 2) ]);
        }
      ?>
    @endif
    ...
    <script type="text/javascript">
      ...
      var iss  = <?php echo json_encode(\Carbon\Carbon::createFromTimeStamp(strtotime($invoice->CreatedDate))->toFormattedDateString()); ?>;
