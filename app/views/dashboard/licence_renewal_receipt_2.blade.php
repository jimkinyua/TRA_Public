...
@section('script')
    ...
    <?php $items = []; $total = $gross = 0; ?>
       ...
       <?php 
    //    echo '<pre>';
		//  print_r($receipt->items()->LicenceRenewalnvoiceLinesID);
		//   Die(); 
		$paid=0;
		$its = $receipt->items()->where('InvoiceHeaderID',$InvoiceNo)->get();
		...
		{                     
			...
			array_push($items, [($receipt->ReferenceNumber), (date('d/m/Y',strtotime($receipt->ReceiptDate))),($Description), number_format($item_amnt, 2)]);       			
		}
		$gross=$total;		
      ?>

    <?php array_push($items, [ '','', 'Gross Total', number_format($gross, 2) ]); ?>
	<?php array_push($items, [ '','', 'Balance', number_format($Balance, 2) ]); ?>
    ...
    <script type="text/javascript">
	  ...
	  var receiptid =<?php echo json_encode($receipt->id()); ?>;
