...
<?php
	...
	define("MPESA_NOT_PAID", 1);
	define("MPESA_PAID", 1);
	define("MPESA_REVERSED", 2);
	...
	class Transaction
	{
		...
		public function txn_is_complete($txn)
		{
			...
			if(isset($txn['paymentmethod']) && $txn['paymentmethod'] === "Mpesa") {
				if(!isset($txn['mpesacode']) || empty($txn['mpesacode'])) {
					$has_error = true;
				}
			}
		...
		private function day_invoice_created($customer) {
			...
			$sql .= "where customerid=(select top 1 agentid from agents where username=? order by agentid desc) order by invoiceheaderid desc) as date) = cast(getdate() as date) ";
		...
		private function create_invoice($customer_id) {
			...
			$sql .= "values (GETDATE(),(select top 1 agentid from agents where username=? order by agentid desc),GETDATE(),(select top 1 agentid from agents where username=? order by agentid desc),0)";
		...
		public function settle_mpesa_pmt($mpesa_code) {
			...
			if($db->run_query($sql, array($mpesa_code))) {
				$row = $db->fetch_assoc();
				/*while($row = $db->fetch_assoc()) {
					print("status: " . json_encode($row));
				}*/
				//die(var_dump($row));
			...
			//echo "OK";
				...
				if($db->run_query($mpesa_application_query)) {
					...
					if(!$this->settle_mpesa_pmt($txn['mpesacode'])) {
						$this->log->info("mpesa code not found");
						return false;
					}
		...
		public function transact($txn) {
			...
			if(true) {
				...
				foreach($txn as $pmt) {
					...
					if(strcmp(strtoupper(trim($pmt['paymentmethod'])), "MPESA") == 0) {
						// validate
						if(!$this->settle_mpesa_pmt($pmt['mpesacode'])) {
							//$this->log->info("mpesa code not found");
							$this->message = "mpesa code not found";
							return false;
						}
					}
					...
					$sql = "insert into invoicelines (invoiceheaderid, serviceid, serviceheaderid, posreceiptid, marketid, amount, ";
					$sql .= "createdate, createdby) values (?,?,?,?,(select top 1 marketid from markets where marketname=?),?,GETDATE(),(select top 1 agentid from agents where username=? order by agentid desc))";
					...
					if($db->run_query($sql, array($invoice_header, $service_id, $service_hdr, $pmt['billno'], $pmt['market'], $pmt['amount'], $pmt['agent'])) && $db->get_affected_rows() >  0) {
						...
						if($db->run_query($sql, array($pmt['userid']))) {
							...
							$sql = "update invoiceheader set amount=(select sum(amount) from invoicelines where invoiceheaderid=?) where invoiceheaderid=?";
							
							if($db->run_query($sql, array($invoice_header, $invoice_header))) {
								if($db->get_affected_rows() <=  0) {
									echo __LINE__;
									echo "error updating amount";
								}
							}
						}
						...
						if($invoice_line > 0) {
							...
							if(isset($pmt['params']) && count($pmt['params']) > 0) {
								...
								foreach($pmt['params'] as $key=>$value) {
									...
									$sql .= "(select top 1 serviceheaderid from serviceheader where customerid=(select ca.customerid from customeragents ca ";
									$sql .= "join agents a on ca.agentid=a.agentid where a.username=?) order by serviceheaderid desc), ?,GETDATE(),";
									$sql .= "(select top 1 agentid from agents where username=? order by agentid desc))";
		...
		public function calc_eod($username = NULL) {
			if($username != NULL && is_string($username)) {
				global $db;
				$sql = "select top 1 ih.invoiceheaderid as invoiceNo, CONVERT(NVARCHAR, ih.invoicedate, 120) as invoicedate,(select sum(amount) from invoicelines ";
				$sql .= "where invoiceheaderid=ih.invoiceheaderid) as total ";
				$sql .= "from invoiceheader ih where createdby = (select top 1 agentid from agents where username=? order by agentid desc) and paid=0";
